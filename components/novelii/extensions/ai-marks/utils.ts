...
/**
 * Get metadata of active AI mark at selection or cursor (returns reason string or null)
 */
export function getAiMarkMetadata(editor: Editor): string | null {
  ...
  editor.state.doc.nodesBetween(from, to, node => {
    node.marks.forEach(mark => {
      if (AI_MARKS.includes(mark.type.name)) {
        found = mark.attrs?.reason || null
      }
    })
  })

  return found // e.g., "AI grammar improvement."
}
